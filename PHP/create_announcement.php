...
$locationsToInsert = [];
if ($locationName === "All") {
    $locationsToInsert = array_values($locations); // all IDs
} else {
    ...
    $locationsToInsert = [$locations[$locationName]]; // single location
}
...
$stmt = $conn->prepare("
    ...
    VALUES (?, ?, ?, ?)
");
foreach ($locationsToInsert as $locId) {
    $stmt->bind_param("isss", $locId, $message, $start_date, $end_date);
    $stmt->execute();
}
